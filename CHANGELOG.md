...
All notable changes to the Keyrunes server.

Generated by `git-cliff` from the conventional commits. **Do not edit by hand** —
the next run overwrites it. To change the text of an entry, reword the commit
message; to change the shape, edit `cliff.toml`.
...
- *(changelog)* Put cliff.toml and the changelog header in English
