...
Todas as mudanças notáveis do servidor Keyrunes.

Gerado por `git-cliff` a partir dos commits convencionais. **Não edite à mão** —
a próxima geração sobrescreve. Para mudar o texto de uma entrada, reescreva a
mensagem do commit; para mudar a forma, edite `cliff.toml`.

## [unreleased]

### 🐛 Bug Fixes

- *(auth)* Enforce token expiry and refuse privileged groups at registration
- *(deps)* Rustls 0.23.40 → 0.23.45 (RUSTSEC-2026-0285)

### 📚 Documentation

- *(mutants)* Record the known survivors and why each is left alone
- *(changelog)* Gerar o CHANGELOG.md com git-cliff

### 🧪 Testing

- Add exhaustive suites and point the fake tests at the real code
- *(auth)* Cover the middleware gates themselves
- *(logging)* Assert the middleware passes responses through untouched
- *(policy)* Pin the read and assignment paths against their repository
- *(admin)* Drive the admin page handler, not just its gate
...
- *(changelog)* Add v0.3.0 release section
